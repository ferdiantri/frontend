...
import { Text, Image, SafeAreaView, FlatList, View, StyleSheet, ScrollView, ActivityIndicator,TouchableOpacity, TextInput, Alert, ToastAndroid } from 'react-native';
...
export default function KeranjangScreen({ navigation }){
    ...
    const numberFormat = (value) => new Intl.NumberFormat('id-ID', {
        ...
        async function kurangKeranjang(id_barang, jumlah_barang){
            if(jumlah_barang <= 1){
                Alert.alert('Minimal Pembelian 1');
            }
            else{
                let headers = {
                    headers: {
