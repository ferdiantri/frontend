...
export default function PesananDikomplainScreen ({navigation}){
    ...
    async function onRefresh(){
        ...
        try{  
            ...
            axios.get(`http://192.168.43.22:8000/api/user/penjualan_dikomplain?email=${profile}`, headers).then(response => {
            ...
            if(token == null){
                ...
                try{
                    axios.get('http://192.168.43.22:8000/api/user/profile', headers).then(response => {
                        ...
                        axios.get(`http://192.168.43.22:8000/api/user/penjualan_dikomplain?email=${email}`, headers).then(response => {
                            console.log(response.data.barang)
                            if(response.data.error){
                                setEmpty(true);
                            }
                        ...
                        <View style={{ flexDirection: 'column', flex: 4.5, marginHorizontal: 5, marginVertical: 5 }}>
                            {item.barang.map(data_barang => {
                                ...
                                <View style={{marginHorizontal: 10, flexDirection: 'row'}}>
                                    ...
                                    <View>
                                        <Button onPress={() => {navigation.navigate('DetailKomplainScreen', {id_penjualan: (item.id_penjualan)})}} title='Detail Komplain'/>
                                    </View>
                                </View>
                            </View>
                    </View>     
